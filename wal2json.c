...
typedef struct
{
	 ...
	 */
	bool		change_lsn;			/* include LSNs on each row */
...
pg_decode_startup(LogicalDecodingContext *ctx, OutputPluginOptions *opt, bool is_init)
{
	...
	foreach(option, ctx->output_plugin_options)
	{
		...
		else if (strcmp(elem->defname, "include-change-lsn") == 0)
		{
			if (elem->arg == NULL)
			{
				elog(DEBUG1, "include-change-lsn argument is null");
				data->change_lsn = true;
			}
			else if (!parse_bool(strVal(elem->arg), &data->change_lsn))
				ereport(ERROR,
						(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
						 errmsg("could not parse value \"%s\" for parameter \"%s\"",
							 strVal(elem->arg), elem->defname)));
